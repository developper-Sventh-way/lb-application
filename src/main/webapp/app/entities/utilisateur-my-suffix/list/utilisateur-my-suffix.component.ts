...
export class UtilisateurMySuffixComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.utilisateurService
      ...
      .subscribe({
        next: (res: HttpResponse<IUtilisateurMySuffix[]>) => {
  ...
  trackId(_index: number, item: IUtilisateurMySuffix): number {
    return item.id!;
  }

  delete(utilisateur: IUtilisateurMySuffix): void {
    const modalRef = this.modalService.open(UtilisateurMySuffixDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IUtilisateurMySuffix[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/utilisateur-my-suffix'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
